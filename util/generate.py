...
def generate_fast(
    ...
    if output_top5_tokens:
        top5 = torch.topk(softmax_out, 5, dim=1)
        return txt, top5.indices, top5.values
    return txt
...
def generate_standard(model, prompt, tokenizer, max_input_tokens=256, max_new_tokens=64, top_p=0.9, temperature=0.7, do_sample=False, top_k=50):
    ...
    #prompt是12个句子开头的第一个词，下面是开始续写，按理来说，续写不需要什么padding啊，不知道搞什么鬼。反正，暂时是没有填充的。
    with torch.no_grad():
    ...
    s = outputs.sequences#[12,11]，除去开头一个，生成了10个新的词。
    outputs = tokenizer.batch_decode(s, skip_special_tokens=True, \
        clean_up_tokenization_spaces=True)#将token id还原成单词，得到的12句话，每句话11个单词。
    results = [output for output in outputs]#这个是垃圾语句，没有起到任何作用，就是自身。

    if right_pad_flag:#不会进入，已经设置为了左填充。
