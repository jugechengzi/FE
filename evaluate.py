...
from memit.memit_edited import apply_memit_to_model_edited
from memit.memit_a_main import apply_memit_a_to_model
...
ALG_DICT = {
    "NONE": (None, None),
    "MEMIT_A": (MEMITHyperParams, apply_memit_a_to_model),
    "MEMIT_EDITED": (MEMITHyperParams, apply_memit_to_model_edited),
...
def main(
    ...
    apply_division: bool = True,
    skip_evaluation: bool = False,
):
    if alg_name == "NONE":
        print("No editing algorithm specified. Directly apply original model for evaluation.")
    ...
    if alg_name == "NONE":
        print("Skip loading hyperparameters for NONE algorithm.")
        hparams = None
    else:
    ...
    cache_template = None
    if use_cache:
        # 统一 MEMIT 和 MEMIT_EDITED 的 kvs 路径，不区分算法名
        if alg_name in ["MEMIT", "MEMIT_EDITED"]:
            print("Using unified cache path for MEMIT and MEMIT_EDITED, since they share the same k/v pairs.")
            alg_name = "MEMIT"
            
        # 统一 MEMIT(们) 和 AlphaEdit 的 kvs 路径，不区分算法名
        # 注意：新加入的MEMIT_A使用自己的缓存路径，因为它只需要计算第一层的k/v对，而MEMIT需要计算最后一层的k/v对，MEMIT_EDITED需要计算所有修改层的k/v对
        # 这样的话MEMIT的缓存路径应当有所有修改层的K/V对，MEMIT_A的缓存路径只需要求第一层的K/V对，然后通过前向传播得到后续层的K/V对
        # 为了对比前向传播的K/V对和单独每层的K/V对的差异，还是分开存储比较好
        # 但是可以把MEMIT_A缓存的第一层信息复用到MEMIT和MEMIT_EDITED中，避免重复计算
        # 此处逻辑容易混淆，后续再细化，如需复用可以单独写脚本将文件从MEMIT_A目录复制到MEMIT目录
        if alg_name == "MEMIT_A":
            cache_template = (
                KV_DIR
                / f"{model_name.replace('/', '_')}_MEMIT_A"
                / f"{ds_name}_layer_{{}}_clamp_{{}}_case_{{}}.npz"
            )
        elif any(alg in alg_name for alg in ["MEMIT","AlphaEdit", "MEMIT_seq", "MEMIT_prune", "MEMIT_rect"]):
            cache_template = (
                KV_DIR
            ...
            # 例如：MEMIT_A的路径：<model_name>_MEMIT_A/<ds_name>_layer_{}_clamp_{}_case_{}.npz
            cache_template = (
                KV_DIR
    ...
    for record_chunks in chunks(ds, num_edits):
        ...
        elif alg_name == "MEMIT_A":
            print("Applying MEMIT_A with apply_division =", apply_division)
            edited_model, _ = apply_algo(
                model,
                tok,
                    [
                    ...
                    hparams,
                    return_orig_weights=False,
                    apply_division=apply_division, # specific to MEMIT_A
                    **args_conserve_memory,
                    **etc_args,
                )
        elif alg_name == "NONE":
            edited_model = deepcopy(model) # No editing, just copy the original model, use deepcopy to avoid any accidental modifications
        else:
            edited_model, _ = apply_algo(
                model,
                tok,
    ...
    if skip_evaluation:
        print("Skipping evaluation. Exiting.")
        exit(0) # Early exit, 用exit不用return是为了彻底结束程序，避免后续代码执行
    # 此处不会影响原来的存储的模型，因为edited_model是新返回的模型，这一步不会修改原来的model，但是evaluate后面的代码会用到edited_model

    # Iterate through dataset again for evaluation
    start = time()
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--alg_name",
        choices=["AlphaEdit","MEMIT_rect", "MEMIT_seq","MEMIT_prune", "MEMIT", "ROME", "FT", "MEND","NSE", "MEMIT_EDITED", "MEMIT_A","NONE"],
        default="ROME",
    ...
    parser.add_argument(
        "--apply_division",
        action="store_true",
        help="In MEMIT_A, whether to apply division when computing residual, i.e. r = targets / layers or r = targets"
    )
    parser.add_argument(
        "--skip_evaluation",
        dest="skip_evaluation",
        action="store_true",
        help="Skip evaluation after performing edits.",
    )
    parser.set_defaults(skip_generation_tests=False, conserve_memory=False, skip_evaluation=False)
    ...
    main(
        ...
        apply_division=args.apply_division,
