...
def get_reprs_at_word_tokens(
    ...
    try:
        # 正常情况下，context_templates是模板，需要替换subject
        ...
        # 特殊情况下（negetive_prompt_test时），context_templates是完整的prompt不需要替换subject
        return get_reprs_at_idxs(
            model,
            tok,
            context_templates,
...
def get_words_idxs_in_templates(
    ...
    for i, context in enumerate(context_templates):
        try:
            # 正常情况下，context_是模板，需要替换subject，因此有{}
            prefix, suffix = context.split("{}")
        except:
            # 特殊情况下（negetive_prompt_test时），context是完整的prompt，没有{}，但应包含subject，因此用subject进行split
            contexts = context.split(words[i])
            if len(contexts) > 2:
                # 偶尔有分开后大于两段的情况
                print(f"length of contexts: {len(contexts)}")
                print(f"prompt: {context}\nsubject: {words[i]}")
                prefix = contexts[0]
                suffix = words[i].join(contexts[1:])
            elif len(contexts) <= 1:
                print(f"length of contexts: {len(contexts)}")
                print(f"prompt: {context}\nsubject: {words[i]}")
                raise ValueError(f"length of contexts <= 1")
            else:
                prefix, suffix = context.split(words[i])
    ...
    if subtoken == "last" or subtoken == "first_after_last":
        ...
        return [[inputs_len[i]-1] for i in range(len(context_templates))]
    else:
