...
def apply_pmet_to_model(
    ...
    v_lr = getattr(cfg.llms, 'v_lr', None)
    step = getattr(cfg.llms, 'v_num_grad_steps', None)

    print(f"\nLoading full z cache for PMET...")
    cache_zs_file = f"{cfg.zs_cache_dir}/{dataset_name}-{model_cache_name}-{z_method}-seed{seed_value}"
    if v_lr is not None and step is not None:
        cache_zs_file += f"-vlr{v_lr}-steps{step}"
    cache_zs_file += f"-layer{z_layer}.pt"
