...
def compute_z(
    ...
    '''改动之处'''
    target_init_attn=None
    target_init_layer=None
    out_tuple=None
    ...
    def edit_output_fn(cur_out, cur_layer):
        ...
        if cur_layer == cfg.llms.rewrite_module_tmp.format(layer):#目前来看，这个和下面这个结果是一模一样的。
        # if cur_layer == cfg.llms.mlp_module_tmp.format(layer):
        ...
        '''改动之处'''
        if cur_layer == cfg.llms.attn_module_tmp.format(layer):
            # Store initial value of the vector of interest
            ...
            for i, idx in enumerate(lookup_idxs):
                ...
                if len(lookup_idxs)!=len(cur_out[0]):#batch size在seqlen后的情况，一般不是。
                    cur_out[0][idx, i, :] += delta_attn
                else:
                    cur_out[0][i, idx, :] += delta_attn
        '''改动之处'''
        if cur_layer == cfg.llms.layer_module_tmp.format(layer):
            # Store initial value of the vector of interest
            if target_init_layer is None:
                print("Recording initial value of v* in layer")
                # Initial value is recorded for the clean sentence
                target_init_layer = cur_out[0][0, lookup_idxs[0]].detach().clone()
        if not out_tuple:
            cur_out = cur_out[0]
        return cur_out
    ...
    # Optimizer
    '''改动之处'''
    opt = torch.optim.Adam([delta,delta_attn], lr=cfg.llms.v_lr)
    ...
    for it in range(cfg.llms.v_num_grad_steps):
        ...
        with nethook.TraceDict(
            ...
            layers=[
                ...
                # cfg.llms.layer_module_tmp.format(layer),#取消了这个。
                cfg.llms.rewrite_module_tmp.format(layer),  ########
                # cfg.llms.mlp_module_tmp.format(layer),########
                cfg.llms.attn_module_tmp.format(layer),
            ],
            retain_input=False,
            retain_output=True,
        ...
        max_norm = cfg.llms.clamp_norm_factor * target_init_layer.norm()#这里本来是target_init的，但是我改成了layer，前者根本没法训练起来，因为norm被限制得太小。
        if delta.norm() > max_norm:
            with torch.no_grad():
                delta[...] = delta * max_norm / delta.norm()
        max_norm_attn = cfg.llms.clamp_norm_factor * target_init_attn.norm()
        if delta_attn.norm() > max_norm_attn:
            with torch.no_grad():
                delta_attn[...] = delta_attn * max_norm_attn / delta_attn.norm()
