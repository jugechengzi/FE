...
def apply_emmet_to_model(
    ...
    """EmmET (variant): use cached z (z_method=all) from last edit layer."""
...
def batch_edit(cfg, model, tok, requests, device, cache_c, zs_full, z_layer):
    ...
    for i, layer in enumerate(cfg.llms.layers):
        ...
        last_two = torch.linalg.solve(
            coef * cov.to(device) + cache_c[i, :, :].to(device),
            layer_ks.to(upd_type),
        ).T
        KETC01KE = last_two @ layer_ks.to(upd_type)
        KETC01KE_norm = KETC01KE + cfg.algs.inv_coef * torch.eye(
            KETC01KE.shape[0], dtype=KETC01KE.dtype, device=KETC01KE.device
        )
