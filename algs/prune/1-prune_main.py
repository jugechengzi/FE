...
def func_log(original_s, delta_s):
    max_ori = original_s[0]
    C = max_ori - torch.log(max_ori)
    delta_s = torch.where(delta_s >= max_ori, torch.log(delta_s) + C, delta_s)
    return delta_s
...
def func_logn(original_s, delta_s, n):
    ...
    delta_s = torch.where(delta_s >= max_ori, log_n_delta_s + C, delta_s)
    return delta_s


def prune_norm(cfg, orig_matrix, upd_matrix):
    u, s, v = torch.linalg.svd(upd_matrix, full_matrices=False)
    _, s0, _ = torch.linalg.svd(orig_matrix.to(upd_matrix.dtype), full_matrices=False)
    tol = max(upd_matrix.shape) * torch.finfo(s.dtype).eps * s[0]
    rank = (s > tol).sum().item()

    Reduce = cfg.algs.reduce_type
    if Reduce == "linear":
        s2 = func_linear(s0, s)
    elif Reduce == "log":
        s2 = func_log(s0, s)
    ...
    u2 = u[:, :rank]
    s2 = torch.diag(s2[:rank])
    v2 = v[:rank]

    upd_matrix_norm = u2 @ s2 @ v2
...
def apply_prune_to_model(
    ...
    """PRUNE (variant): use cached z (z_method=all) from last edit layer."""
