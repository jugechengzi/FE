        ...
        for i in range(config.n_iter):
            ...
            if self.config.retrieve and self.get_adapter_layer().merge_cnt > 0 and self.config.replay:
                ...
                for _ in merge_group_edit_history:
                    ...
                    memo_input = {f"{k1}" : v1.to(self.model.device) for k1, v1 in memo_input.items()}
        ...
        if self.editing:
            ...
            if not self.config.retrieve:
                ...
                try:
                    ...
                    mask1 = (dist1<threshold) & (dist2<threshold)
                    mask2 = ~mask1 & (dist1 > dist2)
                    mask1 = mask1[:,None,None]
                    mask2 = mask2[:,None,None]
                    layer_out = torch.where(mask1,original_layer_output, torch.where(mask2, layer_output, new_weight_layer_output))
            else:
                original_layer_output = self.original_layer(*args)
                new_weight_layer_output = self.new_weight_forward(*args)
