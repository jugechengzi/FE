...
class BaseEditor:
    ...
    def reset_model(self):
        ...
        device = torch.device("cuda:{}".format(self.config.gpu) if torch.cuda.is_available() else "cpu")
        model_name=self.config.llms.name
        from transformers import AutoModelForCausalLM
        model = AutoModelForCausalLM.from_pretrained(model_name,torch_dtype=self.config.model_dtype,trust_remote_code=True).to(device)
        self.model = make_model(self.config, model)
    ...
    def cache(self, tuples: List[Dict[str, torch.LongTensor]]):
        ...
        for idx, t in enumerate(tuples):
            ...
            for module_idx, module_name in enumerate(self.config.llms.edit_modules):
                ...
                keys = tr[module_name].keys.to(torch.float32).to(self.model.device)
                values_grad = tr[module_name].values_grad.to(torch.float32).to(self.model.device)
                self.net[str(shape)].normalizer.update(torch.cat((keys, values_grad), -1))# update the running mean and std
                dir_path = f"{self.config.cache_dir}/{self.config.llms.name.replace('/', '-')}_rledit_{self.config.data}_{self.config.algs.num_seq}_{self.config.algs.n_edits}"
    ...
    def train(self, loader: DataLoader, save=False):
        ...
        for _, tuples in enumerate(tqdm(loader, desc = "Train", ncols = 100)):
            ...
            for t in tuples["unrel_tuples"]:
                ...
                (self.config.algs.loc_coef * loss).backward()
                self.edit_model(param_shifts, True)
    ...
    def run(self, train_loader: DataLoader, valid_loader: DataLoader):
        ...
        for _ in range(self.config.algs.n_epochs):
            ...
            empty_cache(self.config.cache_dir, self.config)
            self.reset_hypernet()
    ...
    def run_single(self, train_loader: DataLoader, valid_loader: DataLoader):
        ...
        torch.save(self.net.state_dict(), f"checkpoints/{self.config.llms.name.replace('/', '-')}_{self.config.algs.name}__{self.config.dataset.name}_{str(self.config.algs.num_seq)}_{str(self.config.dataset.n_edits)}_net.pth")
        torch.save(self.opt.state_dict(), f"checkpoints/{self.config.llms.name.replace('/', '-')}_{self.config.algs.name}__{self.config.dataset.name}_{str(self.config.algs.num_seq)}_{str(self.config.dataset.n_edits)}_opt.pth")
